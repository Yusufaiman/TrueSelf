...
import { MIND_DIMENSION_ORDER, MIND_DIMENSIONS } from "@/lib/mind-profile/data";
import type { MindProfileResult } from "@/lib/mind-profile/types";

export interface MindResultTemplateProps {
  variant: "mind-profile";
  result: MindProfileResult;
  onRetake: () => void;
}

export default function MindResultTemplate({
  result,
  onRetake,
}: MindResultTemplateProps) {
  const handleDownloadPdf = () => {
    const originalTitle = document.title;
    document.title = `TrueSelf Mind - ${result.pattern.name}`;
    window.print();
    window.setTimeout(() => {
      ...
      <div className="mx-auto max-w-6xl">
        <div className="pdf-hide mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm font-semibold text-slate-500">
            Mind describes cognitive style, not intelligence, diagnosis, or mental health.
          </p>
        ...
        <header className="mb-8 rounded-[2rem] border border-cyan-200 bg-white p-8 text-center shadow-sm md:p-10">
          ...
          <p className="mt-3 text-lg italic text-cyan-600">
        ...
        <section className="mb-8 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7">
          <SectionTitle icon={<Brain size={20} />} title="Your Mind Profile" />
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
            These are cognitive-style dimensions. Analytical and intuitive
            processing are independent signals, not opposite ends of one scale.
          </p>
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {MIND_DIMENSION_ORDER.map((dimension) => {
        ...
        <section className="mb-8 grid gap-6 lg:grid-cols-4">
          <MetricCard title="Processing Style" value={result.processingStyle} icon={<Brain size={20} />} />
          <MetricCard title="Learning Style" value={result.learningStyle} icon={<Lightbulb size={20} />} />
          <MetricCard title="Decision Style" value={result.decisionStyle} icon={<Compass size={20} />} />
          <MetricCard title="Uncertainty" value={result.uncertaintyStyle} icon={<Eye size={20} />} />
        </section>

        <section className="mb-8 grid gap-6 lg:grid-cols-2">
          <Panel title="Cognitive Strengths" icon={<Sparkles size={20} />}>
            <TagList items={result.cognitiveStrengths} />
          </Panel>
          <Panel title="Possible Cognitive Friction" icon={<AlertTriangle size={20} />}>
            <List items={result.frictionPoints} />
          </Panel>
        </section>

        <section className="mb-8 grid gap-6 lg:grid-cols-2">
          <Panel title="Cross-Dimension Insights" icon={<Brain size={20} />}>
            <List
              items={
                result.insights.length
                  ? result.insights
                  : ["Your cognitive profile appears balanced or context-dependent, so stronger insights may emerge as more TrueSelf assessments are completed."]
              }
            />
          </Panel>
        ...
        <section className="mb-8 rounded-[2rem] border border-cyan-100 bg-cyan-50 p-6 md:p-7">
        ...
        <div className="pdf-hide mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          ...
          <Link
            href="/dashboard"
            className="inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-cyan-500 to-teal-500 px-5 py-3 font-semibold text-white shadow-md transition hover:from-cyan-600 hover:to-teal-600"
          >
            View dashboard
            <ArrowRight size={18} />
          </Link>
...
function MetricCard({
  ...
  return (
    <article className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-full bg-cyan-50 text-cyan-600">
        {icon}
      </div>
